...
- **Route: Halhjem-Sandvikvåg only (line 1018).** *My call.* This is the ferry I
  actually take travelling Lillesand to Bergen, so it's the one I care about
...
sailings, test on later, never random, which would leak future weather);
baseline (predict base rate) printed every run so "did the model learn anything"
is always visible; calibrated probabilities scored with Brier / log loss + a
reliability table (accuracy is meaningless on a mostly-on-time target). Target =
"disrupted" (delay >= 3 min OR cancelled), cancellation folded into the positive
class but stored separately so it stays reversible.
...
damped, ~0.0 now and only ~0.3 m max even over 48 h, while open water a few km
seaward (Austevoll, Slåtterøy) runs 1.5 to 1.9 m. Moving the sample point along the
